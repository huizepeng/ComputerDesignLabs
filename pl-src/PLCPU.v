...
module PLCPU(
 ...
 // instantiation of pc unit
	...
	NPC U_NPC(.PC(PC_out), .NPCOp(EX_NPCOp),
	          .JumpPC(npc_base), .IMM(EX_immout), .NPC(NPC));
	EXT U_EXT(
...
// MUX Gate 
    ...
    assign forward_rs1_from_mem = MEM_RegWrite && (MEM_rd != 0) && (MEM_rd == EX_rs1);
    assign forward_rs2_from_mem = MEM_RegWrite && (MEM_rd != 0) && (MEM_rd == EX_rs2);
    assign forward_rs1_from_wb  = WB_RegWrite  && (WB_rd  != 0) && (WB_rd  == EX_rs1) && !forward_rs1_from_mem;
    assign forward_rs2_from_wb  = WB_RegWrite  && (WB_rd  != 0) && (WB_rd  == EX_rs2) && !forward_rs2_from_mem;
    ...
    begin
        ...
        if (forward_rs2_from_mem)
            alu_in2 = MEM_aluout;
        else if (forward_rs2_from_wb)
            alu_in2 = WD;
        else
            alu_in2 = EX_RD2;
    end
    ...
    wire jump_flush;
    assign jump_flush = (EX_NPCOp == `NPC_JUMP) || (EX_NPCOp == `NPC_JALR) || (EX_NPCOp == `NPC_BRANCH);

    wire [31:0] npc_base;
    assign npc_base = (EX_NPCOp == `NPC_JALR) ? EX_RD1 : EX_pc;
